 ...
 *
 * Shared integration test suite for StoreInterface implementations.
 ...
 *   runIntegrationTests('SqliteStore', () => { ... });
 */
...
export function runIntegrationTests(
  storeName: string,
  createStore: () => StoreInterface | Promise<StoreInterface>,
): void {
  test(
    `[${storeName} Integration] add job -> worker process -> completed`,
    async () => {
  ...
  test(`[${storeName} Integration] job retry with backoff`, async () => {
    const store = await createStore();
  ...
  test(`[${storeName} Integration] stalled job recovery`, async () => {
    const store = await createStore();
  ...
  test(`[${storeName} Integration] delayed job promotion`, async () => {
    const store = await createStore();
  ...
  test(
    `[${storeName} Integration] global concurrency via getActiveCount`,
    async () => {
      const store = await createStore();
